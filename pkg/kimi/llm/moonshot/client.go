...
// MoonshotClient calls the OpenAI-compatible Moonshot chat completion API.
...
func (c *MoonshotClient) ModelName() string {
	if c == nil {
		return ""
	}
	return c.model
...
func (c *MoonshotClient) Chat(ctx context.Context, req llm.ChatRequest) (*llm.ChatResponse, error) {
	if c == nil {
		return nil, errors.New("moonshot client: nil")
	}

	payload := c.buildChatCompletionRequest(req, false)
	rawPayload, err := json.Marshal(payload)
	if err != nil {
		return nil, fmt.Errorf("moonshot chat: marshal request: %w", err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&decoded); err != nil {
		return nil, fmt.Errorf("moonshot chat: decode response: %w", err)
	}
	if len(decoded.Choices) == 0 {
		return nil, errors.New("moonshot chat: missing choices in response")
	}
...
func (c *MoonshotClient) ChatStream(ctx context.Context, req llm.ChatRequest) (<-chan llm.ChatEvent, error) {
	if c == nil {
		return nil, errors.New("moonshot client: nil")
	}
	...
	rawPayload, err := json.Marshal(payload)
	if err != nil {
		return nil, fmt.Errorf("moonshot chat stream: marshal request: %w", err)
	}
...
func (c *MoonshotClient) consumeStream(ctx context.Context, body io.ReadCloser, out chan<- llm.ChatEvent) {
	defer close(out)
	defer func() {
		_ = body.Close()
	}()
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(data), &chunk); err != nil {
			c.emitEvent(ctx, out, llm.ChatEvent{Err: fmt.Errorf("moonshot chat stream: decode chunk: %w", err), Done: true})
			return
		}
		if chunk.Usage != nil {
	...
	if err := scanner.Err(); err != nil {
		c.emitEvent(ctx, out, llm.ChatEvent{Err: fmt.Errorf("moonshot chat stream: read stream: %w", err), Done: true})
		return
	}
...
func (c *MoonshotClient) emitPendingToolCalls(ctx context.Context, out chan<- llm.ChatEvent, pending map[int]*streamToolCall) {
	if len(pending) == 0 {
		return
	}
...
func (c *MoonshotClient) emitEvent(ctx context.Context, out chan<- llm.ChatEvent, event llm.ChatEvent) bool {
	select {
	case <-ctx.Done():
		return false
	case out <- event:
...
func (c *MoonshotClient) doRequestWithRetry(ctx context.Context, payload []byte, stream bool) (*http.Response, error) {
	...
	for attempt := 1; attempt <= attempts; attempt++ {
		...
		if err != nil {
			return nil, fmt.Errorf("moonshot request: build request: %w", err)
		}
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			...
			return nil, fmt.Errorf("moonshot request: status %d: %s", resp.StatusCode, message)
		}

		return resp, nil
	}

	return nil, errors.New("moonshot request: exhausted retries")
}
...
func (c *MoonshotClient) buildChatCompletionRequest(req llm.ChatRequest, stream bool) chatCompletionRequest {
