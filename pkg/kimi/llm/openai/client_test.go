...
func TestNewOpenAIClientDefaultsAndFromConfig(t *testing.T) {
	t.Parallel()

	client := NewOpenAIClient(" api-key ", "", " gpt-4o-mini ")
	...
	cfgModel := config.LLMModel{Name: "gpt-4o-mini"}
	fromConfig := NewOpenAIClientFromConfig(cfgProvider, cfgModel)
	if fromConfig.apiKey != "k2" || fromConfig.baseURL != "https://example.com/v1" || fromConfig.model != "gpt-4o-mini" {
		t.Fatalf("NewOpenAIClientFromConfig() mismatch: %#v", fromConfig)
	}
}

func TestOpenAIClientWithModelClonesProvider(t *testing.T) {
	t.Parallel()

	client := NewOpenAIClient("test-key", "https://example.com/v1", "gpt-4o-mini")
	overridden, ok := client.WithModel(" gpt-4.1-mini ").(*OpenAIClient)
	if !ok {
		t.Fatal("WithModel() should return *OpenAIClient")
	}
	if overridden == client {
		t.Fatal("WithModel() returned same pointer, want clone")
	}
	if overridden.model != "gpt-4.1-mini" {
		t.Fatalf("overridden model = %q, want %q", overridden.model, "gpt-4.1-mini")
	}
	if client.model != "gpt-4o-mini" {
		t.Fatalf("original model = %q, want %q", client.model, "gpt-4o-mini")
	}
}
...
func TestOpenAIClientChat(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Model != "gpt-4o-mini" {
			t.Fatalf("request model = %q, want %q", req.Model, "gpt-4o-mini")
		}
	...
	client := NewOpenAIClient("test-key", server.URL+"/v1", "gpt-4o-mini")
	provider, ok := client.WithThinking("high").(*OpenAIClient)
	if !ok {
		t.Fatal("WithThinking() should return *OpenAIClient")
	}
	...
	if got, _ := arguments["q"].(string); got != "openai" {
		t.Fatalf("tool args q = %q, want %q", got, "openai")
	}
}

func TestOpenAIClientChatRequestEncodesAssistantToolCallsAndSkipsThink(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got, _ := args["q"].(string); got != "openai" {
			t.Fatalf("tool arguments q = %q, want openai", got)
		}
		...
		if strings.Contains(string(payload), "\"reasoning_content\"") {
			t.Fatalf("request should not include reasoning_content: %s", payload)
		}
...
func TestOpenAIClientChatStream(t *testing.T) {
	...
	client := NewOpenAIClient("stream-key", server.URL+"/v1", "gpt-4o-mini")
...
func TestOpenAIClientRetryOnRetryableStatus(t *testing.T) {
	t.Parallel()
	...
	client := NewOpenAIClient("retry-key", server.URL+"/v1", "gpt-4o-mini")
...
func TestOpenAIClientNoRetryOnClientError(t *testing.T) {
	t.Parallel()
	...
	client := NewOpenAIClient("bad-request-key", server.URL+"/v1", "gpt-4o-mini")
	client.initialBackoff = time.Millisecond
...
func TestOpenAIClientRetryOnNetworkError(t *testing.T) {
	t.Parallel()
	...
	client := NewOpenAIClient("network-key", server.URL+"/v1", "gpt-4o-mini")
	client.initialBackoff = time.Millisecond
